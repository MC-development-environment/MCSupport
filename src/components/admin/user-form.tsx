...
import { upsertUser } from "@/common/actions/user-actions";
...
import { SkillsManager, UserSkill } from "./skills-manager";
...
const FormSchema = z.object({
  ...
  skills: z.string().optional(), // Habilidades separadas por coma (Inicial)
});

interface UserFormProps {
  user?: Partial<User> & { skills?: string; userSkills?: UserSkill[] };
...
export function UserForm({
  ...
  // Estado local solo para CREACIÓN
  ...
  const form = useForm<z.infer<typeof FormSchema>>({
    ...
    defaultValues: {
      ...
      password: "",
      skills: user?.skills || "", // Para visualización inicial
    },
  });
  ...
  const filteredSuggestions = useMemo(() => {
    const available = catalogSkills.filter(
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormItem>
              ...
              >
                <FormControl>
                  <SelectTrigger>
                    <SelectValue
                      placeholder={t("UserDialog.selectDepartment")}
                    />
                  </SelectTrigger>
                </FormControl>
                <SelectContent>
        ...
        {watchedRole !== "CLIENT" && (
          <FormItem>
            ...
            {/* Si estamos EDITANDO (user.id existe), usamos el Manager con Jerarquía */}
            {user?.id ? (
              <SkillsManager
                userId={user.id}
                initialSkills={user.userSkills || []}
                catalogSkills={catalogSkills}
              />
            ) : (
              /* Si estamos CREANDO, usamos la lógica simple local (sin jerarquía visual aun) */
              <div className="space-y-3">
                ...
                <div className="relative">
                  <FormControl>
                    <Input
                      placeholder={
                        t("UserDialog.skillsPlaceholder") ||
                ...
                <p className="text-xs text-muted-foreground">
                  {t("UserDialog.skillsHint") ||
                    "Type skill name and press Enter. Select from suggestions or add new."}
                </p>
              </div>
            )}
            <FormMessage />
