...
import { getAnalytics, AnalyticsPeriod } from "@/common/actions/analytics-actions";
...
export function AnalyticsDashboard({
  ...
  useEffect(() => {
    startTransition(async () => {
      ...
      // Si hay rango de fechas personalizado interno, usarlo
      const result =
        dateRange?.from && dateRange?.to
          ...
          : await getAnalytics(period, userId);
      if (!result.error) {
        setData(result as AnalyticsData);
      }
    });
  }, [period, userId, dateRange, customStartDate, customEndDate]);
      ...
      >
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          {!hideFilters && (
            <>
              <Tabs
                value={dateRange?.from ? "custom" : period}
                onValueChange={(v) => {
                  if (v !== "custom") {
