...
import { VacationBanner } from "@/components/admin/vacation-banner";
...
export default async function AdminLayout({
  ...
  if (
    ...
    session?.user?.role === "VIRTUAL_ASSISTANT"
  ) {
  ...
  return (
    <div className="grid h-screen w-full md:grid-cols-[200px_1fr] lg:grid-cols-[240px_1fr] overflow-hidden">
      ...
      <div className="flex flex-col h-full min-h-0 overflow-hidden">
        {user?.isOnVacation && user.vacationEndDate && (
          <VacationBanner endDate={user.vacationEndDate} />
        )}
