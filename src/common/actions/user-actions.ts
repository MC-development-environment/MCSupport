...
import { welcomeWithPasswordEmail } from "@/infrastructure/services/email/email-templates";
...
export async function getUsers(
  ...
  const usersWithSkills = users.map((user) => ({
    ...user,
    skills: user.skills.map((us) => us.skill.name).join(", "),
...
// Helper interno actualizado para incluir assignedById si es necesario
async function initialSkillAssignment(
  userId: string,
  skillNames: string[],
  assignerId: string,
) {
...
export async function upsertUser(values: z.infer<typeof UserSchema>) {
  ...
  try {
    if (id) {
      ...
      // NOTA: Ya no sincronizamos habilidades aquí para respetar la jerarquía.
      // Las habilidades se gestionarán de forma individual en el componente SkillsManager.
    } else {
      // Crear
      ...
      // Después de crear, asignación inicial de habilidades (si hubiese)
      if (skillsArray.length > 0 && role !== "CLIENT" && session?.user?.id) {
        const newUser = await prisma.user.findUnique({ where: { email } });
        if (newUser) {
          await initialSkillAssignment(
            newUser.id,
            skillsArray,
            session.user.id,
          );
    ...
    return { error: `Error detalle: ${(error as Error).message}` };
  }
}
