                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="secondary" size="sm" disabled={isPending}>
                                    ...
                                    {t('TicketDetail.status')} <ChevronDown className="ml-2 h-4 w-4" />
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end">
                                <DropdownMenuLabel>{t('TicketDetail.status')}</DropdownMenuLabel>
                                <DropdownMenuSeparator />
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="max-h-[300px] overflow-y-auto">
                                <DropdownMenuLabel>{t('TicketDetail.assignee')}</DropdownMenuLabel>
                                <DropdownMenuSeparator />
                                {users.map((u) => (
                                    <DropdownMenuItem key={u.id} onClick={() => u.id && handleBulkAssign(u.id)}>
                                        {u.name || u.email}
                                    </DropdownMenuItem>
                                ))}
                            </DropdownMenuContent>
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className="hidden lg:table-cell whitespace-nowrap">
                                <SortableHeader column="category" label={t('category')} currentSort={sort} currentOrder={order} />
                            </TableHead>
                            <TableHead className="whitespace-nowrap">
                                <SortableHeader column="assignedTo" label={t('TicketDetail.assignee')} currentSort={sort} currentOrder={order} />
                            </TableHead>
                    ...
                    <TableBody>
                        {tickets.length === 0 ? (
                            ...
                            tickets.map((ticket) => (
                                <TableRow key={ticket.id} data-state={selectedIds.has(ticket.id) ? "selected" : undefined}>
                                    <TableCell>
                                        <Checkbox 
                                    ...
                                    <TableCell className="hidden md:table-cell text-right whitespace-nowrap text-muted-foreground">
                                        ...
                                        <span className="block text-xs">{format(new Date(ticket.createdAt), 'HH:mm')}</span>
                                    </TableCell>
                                </TableRow>
                            ))
                        )}
