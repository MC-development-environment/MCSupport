...
import { reopenTicket, getDepartmentsWithUsers } from '@/actions/ticket-actions'
...
export function ReopenTicketDialog({ ticketId, locale = 'es' }: ReopenTicketDialogProps) {
    ...
    const t = {
        ...
        desc: locale === 'es' ? 'Seleccione departamento, usuario y motivo para reasignar' : 'Select department, user and reason to reassign',
        selectDept: locale === 'es' ? 'Seleccionar departamento' : 'Select department',
        selectUser: locale === 'es' ? 'Seleccionar usuario' : 'Select user',
        reasonLabel: locale === 'es' ? 'Motivo de reapertura' : 'Reopen Reason',
        reasonPlaceholder: locale === 'es' ? 'Escribe el motivo...' : 'Enter reason...',
        confirm: locale === 'es' ? 'Reabrir' : 'Reopen',
        success: locale === 'es' ? 'Caso reabierto exitosamente' : 'Case reopened successfully',
        error: locale === 'es' ? 'Error al reabrir el caso' : 'Failed to reopen case'
    }
    ...
    const usersInDepartment = departments.find(d => d.id === selectedDepartment)?.users || []

    const handleReopen = () => {
        if (!selectedUser) return
        if (!reason.trim()) {
            toast.error(locale === 'es' ? 'El motivo es requerido' : 'Reason is required')
            return
        }
