            ...
            <CardContent className="flex-1 flex flex-col gap-4">
                ...
                >
                    ...
                    {ticketDescription && ticketUser && (
                        <div className="sticky top-0 z-10 pb-2 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
                             <div className="flex gap-3 flex-row border-b pb-4 mb-2">
                                <Avatar className="h-8 w-8 mt-1">
                                    <AvatarFallback className="bg-primary/10 text-primary">
                                        {(ticketUser.name?.[0] || ticketUser.email[0] || '?').toUpperCase()}
                                    </AvatarFallback>
                                </Avatar>
                                <div className="flex flex-col items-start max-w-[90%]">
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className="text-[10px] text-muted-foreground">
                                            {ticketCreatedAt && format(new Date(ticketCreatedAt), "dd/MM HH:mm")}
                                        </span>
                                    </div>
                                    <div className="rounded-lg p-3 text-sm bg-muted/50 border border-border/50 text-foreground w-full shadow-sm">
                                        <div className="whitespace-pre-wrap">{ticketDescription}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            return (
                                ...
                                >
                                    <Avatar className="h-8 w-8">
                                        <AvatarFallback className={msg.sender.role === 'VIRTUAL_ASSISTANT' || msg.sender.email === 'assistant@multicomputos.com' ? 'bg-primary text-primary-foreground' : ''}>
                                    ...
                                    >
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            {isInternalMsg && (
                                                <span className="text-[10px] font-bold bg-amber-100 text-amber-800 px-1 rounded border border-amber-200">
                                                    INTERNAL
                                                </span>
                                            )}
                ...
                {!disabled ? (
                         ...
                         {isStaff && (
                            <div className="flex items-center gap-2 pb-2">
                                <label className="flex items-center gap-2 text-sm text-muted-foreground cursor-pointer select-none">
                                    ...
                                    />
                                    <span className={isInternal ? "font-semibold text-amber-700" : ""}>
                                        Nota Interna (Privada)
                                    </span>
                                </label>
                            </div>
                        )}
                        <div className={`flex gap-2 ${isInternal ? "p-1 bg-amber-50/50 rounded-md -m-1" : ""}`}>
                            <Textarea
                                value={content}
                                onChange={(e) => setContent(e.target.value)}
                                placeholder={isInternal ? "Escribe una nota interna..." : "Escribe un mensaje..."}
                                className={`min-h-[80px] ${isInternal ? "border-amber-200 focus-visible:ring-amber-500" : ""}`}
                                onKeyDown={(e) => {
                            ...
                            <Button
                                size="icon"
                                className={`h-[80px] w-[80px] ${isInternal ? "bg-amber-600 hover:bg-amber-700" : ""}`}
                    ...
                    <div className="mt-auto pt-4 border-t text-center text-muted-foreground text-sm">
                        {locale === 'es' ? 'Este caso está cerrado.' : 'This case is closed.'}
                    </div>
                )}
            </CardContent>
        </Card>
